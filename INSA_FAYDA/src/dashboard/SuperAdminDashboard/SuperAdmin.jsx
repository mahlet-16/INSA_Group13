...
function SuperAdmin() {
  ...
  const fetchAdminProfile = async () => {
    try {
      ...
      if (res.data.avatar)
        setProfileImgUrl(`http://localhost:5000/uploads/${res.data.avatar}`);
    } catch {}
  };
  ...
  const handleProfileSave = async (e) => {
    ...
    try {
      ...
      await axios.put("http://localhost:5000/admin/profile", data, {
  ...
  const getActivityDetails = (details) => {
    ...
    if (details.id && !details.company_name)
      parts.push(`Company ID: ${details.id}`);
    return parts.join(" · ");
  };
          ...
          <form className="admin-register-form" onSubmit={handleAddAdmin}>
            <div className="admin-register-form-fields">
              <input
                ...
                value={adminAddForm.username}
                onChange={(e) =>
                  setAdminAddForm({ ...adminAddForm, username: e.target.value })
                }
                required
              />
              <input
                ...
                onChange={(e) =>
                  setAdminAddForm({ ...adminAddForm, email: e.target.value })
                }
              />
              <input
                type="password"
                placeholder="Password"
                value={adminAddForm.password}
                onChange={(e) =>
                  setAdminAddForm({ ...adminAddForm, password: e.target.value })
                }
                required
              />
            </div>
            ...
            {adminAddMsg && (
              <div className="admin-message" style={{ marginTop: "0.5rem" }}>
                {adminAddMsg}
              </div>
            )}
          </form>
              ...
              {companies.map((c) => (
                <div className="company-card" key={c.id}>
                  ...
                  <div className="company-card-actions">
                    <button onClick={() => setDetailsCompany(c)}>
                      Details
                    </button>
                    {editId === c.id ? (
                      <>
                      ...
                      <>
                        ...
                        <button onClick={() => openAddUser(c.id)}>
                          Add User
                        </button>
                      </>
                    )}
